...
app.use(session({
  ...
  app.post('/api/create-subscription', async (req: Request, res: Response) => {
    try {
      ...
      // Stripe price IDs for each plan (these need to be set in your Stripe dashboard)
      const stripePriceIds = {
        'personal': process.env.STRIPE_PERSONAL_PRICE_ID || 'price_personal_default',
        'pro': process.env.STRIPE_PRO_PRICE_ID || 'price_pro_default',
        'deepdive': process.env.STRIPE_DEEPDIVE_PRICE_ID || 'price_deepdive_default'
      };
      
      const selectedPlan = plan?.toLowerCase();
      if (!selectedPlan || !stripePriceIds[selectedPlan as keyof typeof stripePriceIds]) {
        return res.status(400).json({ error: 'Invalid plan selected' });
      }

      // Check if we need to create or update Stripe customer
          ...
          if (existingSubscription.status === 'active') {
            ...
            const updatedSubscription = await stripe.subscriptions.update(user.stripeSubscriptionId, {
              items: [{
                ...
                price: stripePriceIds[selectedPlan as keyof typeof stripePriceIds],
              }],
            ...
            return res.json({
              ...
              clientSecret: null, // No payment needed for updates
              success: true
            });
          }
        } catch (error) {
